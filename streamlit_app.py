    ...
    if st.session_state['ready']:
        ...
        with container:
            with st.form(key='my_form', clear_on_submit=True):
                ...
                # Add buttons
                quick_message_1 = st.button("What is the main topic of the paper?")
                quick_message_2 = st.button("What are the main findings of the paper?")
                quick_message_3 = st.button("What are the implications of the paper?")

            if submit_button and user_input:
                output = await conversational_chat(user_input)
                st.session_state['past'].append(user_input)
                st.session_state['generated'].append(output)
            # Add if statements for buttons
            if quick_message_1:
